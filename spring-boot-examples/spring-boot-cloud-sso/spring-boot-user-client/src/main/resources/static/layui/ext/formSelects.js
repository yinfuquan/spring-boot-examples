    ...
    var $ = layui.jquery || $,
        ...
        commons = {
            ...
            methods: {
                ...
                removeAll: function removeAll(options) {
                    var vals = commons.methods.getValues(options);
                    var diss = commons.methods.getDiv(options).find('dl dd.layui-disabled.xm-select-this').toArray().map(function (dl) {
                        return {val: dl.getAttribute('lay-value')};
                    });
                    var dels = [];
                    for (var i in vals) {
                        if (commons.methods.indexOf(diss, vals[i]) == -1) {
                            dels.push(vals[i]);
                        }
                    }
                    for (var _i in dels) {
                        commons.methods.valHandler(options, dels[_i], false, true);
                    }
                },
                ...
                overrideInput: function overrideInput(options) {
                    ...
                    var _ref5 = [_div.find('.layui-select-title input:first'), $('<div class="layui-input ' + commons.data.name + '"></div>')],
                        $input = _ref5[0],
                        $orinput = _ref5[1];
                ...
                getDom: function getDom(options) {
                    var _ref6 = [commons.data.temps.dom, options.name],
                        _dom = _ref6[0],
                        _name = _ref6[1];
                ...
                getDiv: function getDiv(options) {
                    var _ref7 = [commons.data.temps.div, options.name],
                        _div = _ref7[0],
                        _name = _ref7[1];
                ...
                autoInit: function autoInit(repeat) {
                    $('select[' + commons.data.name + ']').each(function (index, target) {
                        ...
                        if (!repeat) {
                            ...
                            commons.methods.init(commons.methods.cloneOptions(options, hisOptions));
                        } else {
                            commons.methods.init(options);
                        }
                    });
                ...
                listenCloseHandler: function listenCloseHandler(e, that, block) {
                    if (block.length) {
                        var _ref10 = [block.find('.layui-form-select'), block.find('dl')],
                            div = _ref10[0],
                            dl = _ref10[1];
                        ...
                        if (that.attr('fsw') != undefined) {
                            if (that.is('i')) {
                                var _ref11 = [$(e.target).parents('.layui-form-select').prev(), $(e.target).parent()],
                                    sel = _ref11[0],
                                    span = _ref11[1];
                        ...
                        if (that.attr('xm-select-clear') != undefined) {
                            if (that.is('i')) {
                                var _options = commons.data.confs.get($(e.target).attr('xm-select-clear'));
                                if (_options) {
                                    commons.methods.removeAll(_options);
                                }
                                div.addClass('layui-form-selected');
                                dl.find('.xm-select-input-search').focus();
                                return;
        ...
        styles = {
            colors: { //name, spanBgColor, spanColor, iBgColor, iColor, borderColor, tBgColor, tColor
                ...
                primary: ['#009688', '#FFFFFF', '#009688', '#FFFFFF', '#009688'],
